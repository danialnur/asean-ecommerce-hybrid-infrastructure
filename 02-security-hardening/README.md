...
on the same 8-device topology from `../01-regional-on-premises-network/` (`MY-KL-HQ-DIST` excluded — it's an
EtherChannel-only device with no SSH/ACL/port-security surface, see that folder's topology doc; `MY-KL-DMZ-SRV`,
the TFTP/Syslog/DHCP server added mid-phase, is included).
...
- [`evidences/`](evidences/) — live verification screenshots (DHCP Snooping, DAI, NAT, ACLs, NTP/SNMP, TFTP
  backup, DHCP relay) — includes two gaps that were found and fixed live along the way, not just written and
  assumed correct: `SG-EDGE-GW` missing its `WAN-EDGE-INBOUND` ACL, and `Gi1/0/21` missing the DHCP-snooping
  trust that DHCP relay actually depended on to work. See that folder's `README.md` for both.
