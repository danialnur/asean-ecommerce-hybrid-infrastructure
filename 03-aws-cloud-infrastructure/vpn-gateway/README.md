...
**That's the real AWS behavior — this project's own LocalStack evidence doesn't actually show it.** `outputs.tf` correctly requests two distinct attributes (`tunnel1_address`/`tunnel2_address`), but the LocalStack apply captured in `../evidences/README.md` returned the *same* IP for both. That's a LocalStack VPN-emulation limitation (confirmed by cross-checking against the raw `describe-vpn-connections` output, which does show two genuinely different tunnel addresses at the AWS-API level) — not a flaw in this Terraform. Don't treat `vpn_tunnel2_address` from a LocalStack apply as a real second-tunnel peer address; see `evidences/README.md` for the full disclosure.
